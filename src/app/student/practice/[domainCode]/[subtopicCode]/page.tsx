...
export default async function LearnSubtopicPage({ params }: SubtopicPageProps) {
  ...
  const detail = await getSubtopicMasteryDetail(profile.id, subtopicCode, program)

  // Practice-only subtopics (e.g. Reading) have no mastery detail — the domain
  // list is the right place to practise them.
  if (!detail) {
    if (getSubtopic(subtopicCode)) {
      redirect(`/student/practice/${domainCode}`)
    }
    notFound()
  }

  if (detail.domain.code !== domainCode) {
    notFound()
  }
  ...
  const backHref = `/student/practice/${detail.domain.code}`
    ...
    <div className="space-y-6">
        ...
        actions={
          canPractise ? (
            <PracticeSessionSheet
              subtopicCode={mastery.subtopicCode}
              subtopicLabel={mastery.subtopicLabel}
              examType={program}
              availableQuestions={detail.availableQuestions}
              backHref={backHref}
              triggerLabel="Practise this subtopic"
              triggerSize="default"
            />
          ) : null
        }
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard
          ...
          hint={canPractise ? `Published ${program} questions` : 'Nothing available yet'}
      ...
      {!canPractise ? (
        <EmptyState
          ...
          description="It will become practisable as soon as questions with complete answers and diagrams are published for your program."
        />
      ) : null}
              ...
              difficultyPerformance.map((row) => (
                <div key={row.band} className="space-y-1">
                  ...
                  <div className="h-1.5 w-full overflow-hidden rounded-full bg-muted">
                    <div className="h-full rounded-full bg-brand" style={{ width: `${row.accuracy ?? 0}%` }} />
                  </div>
                </div>
              ))
            )}
